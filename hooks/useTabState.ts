...
export function useTabState({
  ...
  const activeTab = useMemo(() => {
    ...
    return param && (validTabs as string[]).includes(param)
      ? param
      : defaultTab;
    ...
    (tab: string) => {
      if (!(validTabs as string[]).includes(tab)) {
        if (process.env.NODE_ENV === "development") {
          console.warn(`[useTabState] "${tab}" is not in validTabs. Ignoring.`);
        }
        return;
      }
  ...
  const isActive = useCallback((tab: string) => tab === activeTab, [activeTab]);
