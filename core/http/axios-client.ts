...
import { type IHttpClient, type HttpResponse, type RequestConfig } from "./http-client";
...
export class AxiosHttpClient implements IHttpClient {
  ...
  constructor(
    ...
    this.instance = axios.create({
      baseURL: `${baseURL}/api/${env.NEXT_PUBLIC_API_VERSION}`,
      timeout: 15_000,
      headers: {
  ...
  private attachResponseInterceptors(): void {
    ...
    this.instance.interceptors.response.use(
      ...
      async (error) => {
        ...
        const shouldRetry =
          RETRY_STATUS_CODES.has(error.response?.status) && retryCount < MAX_RETRIES;
  ...
  private normalizeError(error: unknown): ApiError | NetworkError | TimeoutError {
    if (!isAxiosError(error)) {
      return new ApiError({ message: "Unknown error", code: "UNKNOWN", status: 0 });
    }
  ...
  async request<T>(config: RequestConfig): Promise<HttpResponse<T>> {
    const axiosConfig: AxiosRequestConfig = {
      ...
      onUploadProgress: config.onUploadProgress
        ? (e) => config.onUploadProgress!(Math.round((e.loaded / (e.total ?? 1)) * 100))
        : undefined,
      onDownloadProgress: config.onDownloadProgress
        ? (e) => config.onDownloadProgress!(Math.round((e.loaded / (e.total ?? 1)) * 100))
        : undefined,
    };
  ...
  async download(url: string, filename: string): Promise<void> {
    const response = await this.request<Blob>({ url, method: "GET", responseType: "blob" });
